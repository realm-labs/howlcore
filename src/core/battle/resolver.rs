...
pub fn step_turn(state: &mut BattleState) -> BattleOutcome {
    ...
    if !state.has_started {
        ...
        outcome.push_event(BattleEvent::BattleStarted);
...
fn apply_damage(state: &mut BattleState, mut request: DamageRequest, outcome: &mut BattleOutcome) {
    ...
    if let Some(chimera) = state.chimera_mut(request.target) {
        let hp_before = chimera.stats.hp;
        chimera.stats.hp = (chimera.stats.hp - request.amount).max(0);
        outcome.push_event(BattleEvent::DamageApplied {
            target: request.target,
            amount: request.amount,
            hp_before,
            hp_after: chimera.stats.hp,
        });
    }
...
fn add_attack(
    ...
    if let Some(chimera) = state.chimera_mut(target) {
        let before = chimera.stats.attack;
        chimera.stats.attack = (chimera.stats.attack + amount).max(0);
        outcome.push_event(BattleEvent::AttackChanged {
            target,
            amount,
            attack_before: before,
            attack_after: chimera.stats.attack,
        });
    }
}
...
fn queue_summon(
    ...
    outcome.push_event(BattleEvent::ChimeraQueued { side, chimera });
}
...
fn deploy_queued_summons(state: &mut BattleState, side: TeamSide, outcome: &mut BattleOutcome) {
    loop {
        ...
        state.team_mut(side).chimeras.push(chimera.clone());
        let id = BattleChimeraId { side, index };

        outcome.push_event(BattleEvent::ChimeraSummoned {
            chimera: id,
            state: chimera,
        });
